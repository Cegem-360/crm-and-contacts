...
use App\Filament\Resources\Orders\Pages\EditOrder as FilamentEditOrder;
...
final class ViewOrder extends Component implements HasActions, HasSchemas
{
    ...
    {
        $this->order = $order->load(['customer', 'quote']);
    }
    ...
    public function relationManagers(Schema $schema): Schema
    {
        ...
        $managerLivewireData = [
            ...
            'pageClass' => FilamentEditOrder::class,
        ];
        ...
        if (count($managers) > 1) {
            $tabs = collect($managers)
                ->map(function (string|RelationGroup|RelationManagerConfiguration $manager) use ($managerLivewireData): Tab {
                    ...
                    return $normalizedManagerClass::getTabComponent($this->order, FilamentEditOrder::class)
                        ->schema(fn (): array => [
    ...
    private function getRelationManagers(): array
    {
        return array_filter(
            ...
            fn (string|RelationGroup|RelationManagerConfiguration $manager): bool => $this->normalizeRelationManagerClass($manager)::canViewForRecord($this->order, FilamentEditOrder::class),
        );
    }
