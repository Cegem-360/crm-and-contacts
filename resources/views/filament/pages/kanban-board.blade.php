<x-filament-panels::page>
    {{-- Filters --}}
    <div
        class="flex flex-wrap items-end gap-4 rounded-xl bg-white dark:bg-gray-800 p-4 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700">
        ...
        <div class="min-w-[120px]">
            <label
                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{{ __('Min Value') }}</label>
            <input type="number" wire:model.live.debounce.500ms="filterMinValue" placeholder="0"
                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
        </div>
        <div class="min-w-[120px]">
            <label
                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{{ __('Max Value') }}</label>
    ...
    {{-- Kanban Board --}}
    <div class="flex gap-4 overflow-x-auto pb-4" x-data="kanbanBoard()" x-init="initSortable()">
        @foreach ($stages as $stageValue => $stage)
            <div class="shrink-0 w-72">
                {{-- Stage Header --}}
                <div
                    class="mb-3 rounded-xl bg-white dark:bg-gray-800 p-3 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700">
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="inline-flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 px-2 py-0.5 text-xs font-medium text-gray-600 dark:text-gray-300">
                            {{ $stage['count'] }}
                        </span>
                    </div>
                ...
                {{-- Cards Container --}}
                <div class="kanban-column space-y-3 min-h-[200px] rounded-xl bg-gray-100/50 dark:bg-gray-800/30 p-2"
                    data-stage="{{ $stageValue }}">
                    @foreach ($stage['opportunities'] as $opp)
                        <div class="kanban-card cursor-grab rounded-lg bg-white dark:bg-gray-800 p-3 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 hover:shadow-md transition-shadow"
                            data-id="{{ $opp['id'] }}">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0 flex-1">
                                    <a href="{{ $this->getEditUrl($opp['id']) }}"
