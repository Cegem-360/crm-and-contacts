<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-heading">{{ $complaint->title ?? __('Complaint') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Complaint details') }}</p>
            </div>
        </div>
        <a href="{{ route('dashboard.complaints.edit', ['team' => $currentTeam, 'complaint' => $complaint]) }}"
